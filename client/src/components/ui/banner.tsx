...
const Banner: FC<PropsWithChildren<BannerProps>> = ({ children, onClose, close = true, variant = "info", visible = true }) => {
	if(!visible) return null;

	const Icon = variantIconMap[variant];

	return (
		<div className={`w-full flex items-center gap-1 p-3 border-2 ${variantBorderColorMap[variant]} ${variantBackgroundColorMap[variant]}`}>
			{<Icon />}
			<p className="flex-1 text-start text-sm">{ children }</p>
			{close && <button type="button" onClick={() => onClose?.()}>
				<IoClose />
			</button>}
		</div>
	);
